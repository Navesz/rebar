...
`tooling/rebar-check/index.mjs`, <!--n lines.rebar-check-->3.842<!--/n--> lines.
...
### 4.3 PROVEN · The proofs — <!--n proofs.casos-->82<!--/n--> cases, <!--n proofs.cobertura-->23 de 23<!--/n--> rules

_Remeasured on 31/08/2026 · numbers derived since 02/09/2026._

There are <!--n proofs.casos-->82<!--/n--> cases and <!--n proofs.regras-com-prova-->23<!--/n--> rules with proof — coverage <!--n proofs.cobertura-->23 de 23<!--/n-->, with no uncovered rule. The runner counts the folders
...
Of the <!--n proofs.casos-->82<!--/n-->, **two are from 31/08 and lock the template exclusion**
...
### 4.4 PROVEN · `verificar` — <!--n verify.passos-->28<!--/n--> steps

_Remeasured on 02/09/2026. This file said "8 of 8" and the README said "the 8 steps"; there
were 12 when the count was redone, and there are <!--n verify.passos-->28<!--/n--> now. The
count is now derived from the `default export` of `verify.config.mjs`._

In the order they run:
<!--n verify.lista-passos-->`hygiene` · `hooks` · `commit-msg` · `syntax` · `blocks` · `mcp-server` · `mcp` · `numbers` · `scoreboard` · `format` · `links` · `version` · `secret` · `secret-proofs` · `steps` · `strip` · `proofs` · `generator-map` · `site-paths` · `remote-gate` · `chain` · `generator-identity` · `mcp-template` · `security` · `security-table` · `security-injection` · `security-self` · `self`<!--/n-->

```bash
npm run verify
```

On 02/09/2026: **APROVADO <!--n verify.passos-->28<!--/n--> de <!--n verify.passos-->28<!--/n--> passos · 22,3 s · exit 0.** The duration varies with the
...
| --- | --- | --- |
| `blocos` | <!--n verify.posicao.blocks-->5 de 28<!--/n--> | syntax and `modelo.json` of the files the generator copies into every created project — a defect here is born replicated in all of them |
| `mcp-servidor` | <!--n verify.posicao.mcp-server-->6 de 28<!--/n--> | the MCP server **comes up and answers the protocol**. Without `mcp/node_modules` the step BREAKS (127), it does not fail: missing tooling is not the repository erring |
| `mcp` | <!--n verify.posicao.mcp-->7 de 28<!--/n--> | the MCP artifact diverging from the source — §4.12 |
| `numeros` | <!--n verify.posicao.numbers-->8 de 28<!--/n--> | a number in this file or in the README diverging from the source — §0 |
| `passos` | <!--n verify.posicao.steps-->15 de 28<!--/n--> | the steps that are the **function** of the gate, proven by mutation. `checarBlocos` came in with 410 lines and zero tests, and swapping the body for `return { codigo: 0 }` kept `verificar` APROVADO |
...
`scan-secret.mjs` got ten fixes documented in its own header; the two
...
| `mcp/rules.generated.json`  | **The artifact.** <!--n mcp.artefato.tamanho-->291 KB<!--/n--> · <!--n mcp.artefato.regras-->40<!--/n--> rules · <!--n mcp.artefato.niveis-->8<!--/n--> levels · <!--n mcp.artefato.passos-->28<!--/n--> steps · <!--n mcp.artefato.provas-->203<!--/n--> proofs |
...
`node mcp/generate.mjs` — **one command** — and `verificar` goes back to PASSING the <!--n verify.passos-->28<!--/n--> steps.
...
stays by hand, with the date (§0). It is the <!--n verify.posicao.mcp-->7 de 28<!--/n--> of
...
| ----------------------------------------------------- | ------------------------------------------------------------------------------ |
| `verificar/verificar.mjs`                             | **PRESENT.** 34.566 bytes, tracked. Rewritten, not ported. Today with <!--n verify.passos-->28<!--/n--> steps |
...
<!--n verify.passos-->28<!--/n--> steps
in `verificar`, <!--n proofs.casos-->82<!--/n--> cases in `provar`, <!--n domain.privilegio.testes-->16<!--/n--> assertions in the privilege domain, and 56
...
| 9   | **PLACEHOLDER turning off the whole line.** Measured: 8 of 9 **real** credentials passed. The worst case was `{ host: "localhost", token: "ghp_…" }`, a line any project writes                                                                                                                                                            | The placeholder is tested against the **matched stretch**, never against the line. A finding is released only through the explicit escape hatch `rebar-segredo-ok:`, right after the finding on the same line and within 256 characters, never the whole line. fix 2 of the header of `scan-secret.mjs`                                                                                                   |
...
n/a** (§4.2), `verificar` closes <!--n verify.passos-->28<!--/n--> of <!--n verify.passos-->28<!--/n--> steps, the proofs are <!--n proofs.casos-->82<!--/n--> cases covering <!--n proofs.cobertura-->23 de 23<!--/n--> rules, the hooks are installed, twelve attacks

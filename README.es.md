...
> repositorio, un escáner de firmas conocidas de prompt injection en los archivos que versiona,
> una compuerta que bloquea el commit cuando se ignora una regla, y un generador que construye
> el próximo proyecto ya del lado correcto de la regla.
...
[![Reglas](https://img.shields.io/badge/reglas-40-blue)](#qué-verifica)
[![Compuerta](https://img.shields.io/badge/compuerta-27%20pasos-blue)](#la-compuerta)
...
![rebar-check corriendo sobre el propio repositorio de rebar](docs/assets/rebar-scoreboard.svg)

Esa es la salida real, dibujada de la ejecución por `node tooling/scoreboard.mjs` — y el paso
`scoreboard` de la compuerta falla cuando la imagen deja de coincidir con lo que imprime `npx`.
...
npx -p github:Navesz/rebar rebar-security .   # firmas conocidas de prompt injection
npx github:Navesz/rebar new padaria-do-ze     # empezar del lado correcto
```

Cero dependencias en tiempo de ejecución, Node 22 o más nuevo. El verificador nunca escribe en
...
Son <!--n rules.total-->23<!--/n--> reglas en dos clases. La regla de seguridad suma
<!--n rules.security-->17<!--/n--> más ([las tres reglas](#las-tres-reglas)), y por eso la insignia dice
<!--n rules.all-->40<!--/n-->.
...
Son <!--n proofs.casos-->72<!--/n--> casos, un par por regla, y las <!--n rules.total-->23<!--/n--> reglas están cubiertas:
...
antes que él, hoy con <!--n verify.passos-->27<!--/n--> pasos.

En orden: <!--n verify.lista-passos-->`hygiene` · `hooks` · `commit-msg` · `syntax` · `blocks` · `mcp-server` · `mcp` · `numbers` · `scoreboard` · `format` · `links` · `secret` · `secret-proofs` · `steps` · `strip` · `proofs` · `generator-map` · `site-paths` · `remote-gate` · `chain` · `generator-identity` · `mcp-template` · `security` · `security-table` · `security-injection` · `security-self` · `self`<!--/n-->
...
Lleva <!--n mcp.artefato.regras-->40<!--/n--> reglas de dos módulos, <!--n mcp.artefato.passos-->27<!--/n--> pasos de compuerta y <!--n mcp.artefato.provas-->190<!--/n--> pruebas, expuestos en <!--n mcp.ferramentas-->5<!--/n--> herramientas.

## Mapa del repositorio

| Ruta | Qué |
|---|---|
| `tooling/rebar-check/` | la regla de formato y sus <!--n proofs.casos-->72<!--/n--> casos de prueba |
...
uno. La contribución útil hoy es ejecutar la regla contra un repositorio tuyo y [abrir un issue](https://github.com/Navesz/rebar/issues/new?template=false-positive.yml)

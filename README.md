...
roda antes dele, hoje com <!--n verify.passos-->16<!--/n--> passos. O checker é o último
deles.
...
### Os <!--n verify.passos-->16<!--/n--> passos, e o que cada um barra
...
de baixo: <!--n verify.lista-passos-->`hygiene` · `hooks` · `syntax` · `blocks` · `mcp-server` · `mcp` · `numbers` · `format` · `links` · `secret` · `secret-proofs` · `steps` · `proofs` · `generator-map` · `security` · `self`<!--/n-->

| # | Passo | O que ele barra |
|---|---|---|
| <!--n verify.posicao.hygiene-->1 de 16<!--/n--> | `higiene` | o estado do git contra o que o portão está prestes a afirmar: bit de `skip-worktree`/`assume-unchanged` no índice, ignore local em `.git/info/exclude`, e o hash dos dois arquivos do próprio portão contra o HEAD |
| <!--n verify.posicao.hooks-->2 de 16<!--/n--> | `hooks` | os dois hooks existem e `core.hooksPath` aponta para eles |
| <!--n verify.posicao.syntax-->3 de 16<!--/n--> | `sintaxe` | `node --check` em cada `.mjs` que o git conhece, rastreado ou recém-escrito |
| <!--n verify.posicao.blocks-->4 de 16<!--/n--> | `blocos` | sintaxe e `modelo.json` dos arquivos que o gerador copia para **dentro** de todo projeto criado |
| <!--n verify.posicao.mcp-server-->5 de 16<!--/n--> | `mcp-servidor` | o servidor MCP **sobe e responde ao protocolo** — não basta existir no disco |
| <!--n verify.posicao.mcp-->6 de 16<!--/n--> | `mcp` | o artefato do MCP divergir da fonte de que ele deriva |
| <!--n verify.posicao.numbers-->7 de 16<!--/n--> | `numeros` | um número do README ou do ESTADO divergir da fonte — é o passo que escreveu os números desta página |
| <!--n verify.posicao.format-->8 de 16<!--/n--> | `formato` | `prettier --check .` |
| <!--n verify.posicao.links-->9 de 16<!--/n--> | `elos` | link relativo quebrado na documentação |
| <!--n verify.posicao.secret-->10 de 16<!--/n--> | `segredo` | credencial no repositório, com o conteúdo lido do **índice** quando é `--staged` |
| <!--n verify.posicao.steps-->12 de 16<!--/n--> | `passos` | os passos que são função do portão, provados **por mutação** — o portão provando o portão |
| <!--n verify.posicao.proofs-->13 de 16<!--/n--> | `provas` | os <!--n proofs.casos-->54<!--/n--> casos das regras |
| <!--n verify.posicao.self-->16 de 16<!--/n--> | `auto` | o `rebar-check` apontado para o próprio rebar |
...
O segundo comando é o **passo `mcp` do portão**, o <!--n verify.posicao.mcp-->6 de 16<!--/n--> da lista, e custa ~200 ms nesta máquina
...
mudou, e o quarto volta a APROVAR os <!--n verify.passos-->16<!--/n--> passos:
...
| `mcp/rules.generated.json` | <!--n mcp.artefato.tamanho-->91 KB<!--/n--> · <!--n mcp.artefato.regras-->26<!--/n--> regras · <!--n mcp.artefato.niveis-->8<!--/n--> níveis · <!--n mcp.artefato.passos-->16<!--/n--> passos · <!--n mcp.artefato.provas-->57<!--/n--> provas |
...
| O portão | **funciona** — <!--n verify.passos-->16<!--/n--> passos, CI verde nos dois sistemas, merge barrado com PR plantado |
...
gerador copia para dentro do projeto criado — <!--n new.arquivos-modelo-->24<!--/n--> dos <!--n new.arquivos-->29<!--/n--> arquivos de `new/`. Cada uma das duas pastas tem um

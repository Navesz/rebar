...
const analisar = (arquivos) => achadosDosWorkflows(new Map(Object.entries(arquivos)), TABELAS)
...
describe('files the rule cannot judge step by step', () => {
  test('a gh-aw lock file is a note only when its role check admits every account', () => {
    ...
    assert.deepEqual(vereditos(lock('admin,maintainer,write')), [])
    assert.match(
      textoDe(lock('all')),
      /w\.yml:1:1 generated by gh-aw with its role check open to every account/,
    )
  })
...
function repo(arquivos) {
  ...
  const registros = entradas.map(([caminho, texto]) => {
    const bytes = Buffer.from(texto, 'utf8')

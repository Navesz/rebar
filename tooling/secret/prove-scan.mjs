...
// header) and each of the 22 tests below it failed on the scanner before the fix,
// except the false-positive controls, which say so. Variants live in a TABLE
// inside one test and never in a loop of `test()`: tooling/numbers.mjs counts
// `test(` lines to print how many proofs this file holds.
//
...
function varrer(dir, argumentos) {
  ...
  return { stdout: r.stdout ?? '', stderr: r.stderr ?? '', codigo: r.status ?? 1 }
}
 ...
 */
async function comArquivos(arquivos, { rastreado = false, json = false, depoisDoAdd } = {}) {
  ...
  try {
    ...
    const texto = varrer(dir, modo)
    const resultado = { saida: `${texto.stdout}${texto.stderr}`, codigo: texto.codigo, json: null }
    if (json) {
...
describe(
  ...
  { concurrency: 4 },
  () => {
    test('FINDS · 8 invisible variants inside a token, plus a credential assignment', async () => {
    ...
    test('PRINTS · the label of the hidden code point, never the code point itself', async () => {
      ...
      // the redacted excerpt prints would carry it raw.
      const valor = 'Tr0ub4' + 'dor3xyz'
      const r = await comArquivos({

...
function poolDeRuntime(extra = {}) {
  return new pg.Pool({
    ...
    user: 'app_login', password: 'app_dev_only',
    max: 4,
...
async function comPoolDescartavel(fn) {
  ...
  try { await fn(p) } finally { await p.end() }
}

let runtime

before(() => { runtime = poolDeRuntime() })
after(async () => { await runtime?.end() })

// ─────────────────────────────────────────────────────── positivo

test('a conexão de runtime tem session_user app_login e current_user app', async () => {
  const { rows } = await runtime.query('SELECT session_user, current_user, current_setting($1) AS tz', ['TimeZone'])
...
test('o trigger incrementa version sem o UPDATE mencioná-la', async () => {
  ...
  try {
    ...
    const ins = await c.query(
      'INSERT INTO pedido (tenant_id, descricao) VALUES ($1,$2) RETURNING id, version', ['acme', 'v'])
    const { id, version } = ins.rows[0]
    // Repare: o UPDATE NÃO fala de version. É exatamente o que um agente escreveria.
    const upd = await c.query('UPDATE pedido SET descricao = $1 WHERE id = $2 RETURNING version', ['w', id])
    assert.equal(Number(upd.rows[0].version), Number(version) + 1,
...
test('HOSTIL · RESET ROLE não eleva privilégio: cai em app_login, que não lê nada', async () => {
  ...
  try {
    ...
    assert.equal(rows[0].current_user, 'app_login', 'RESET tem de cair em app_login, nunca em app')
...
test('HOSTIL · SET ROLE db_owner é recusado', async () => {
  const c = await runtime.connect()
  try {
    await assert.rejects(() => c.query('SET ROLE db_owner'), (e) => e.code === '42501')
  } finally { c.release() }
})
...
test('HOSTIL · não há connection-time setting de role', async () => {
  ...
  try {
    ...
    assert.ok(rows[0].r === '' || rows[0].r === 'none',
...
test('HOSTIL · app não faz DDL', async () => {
  const c = await runtime.connect()
  try {
    await assert.rejects(() => c.query('CREATE TABLE intruso (id int)'), (e) => e.code === '42501')
  } finally { c.release() }
})
...
test('HOSTIL · a SECURITY DEFINER não é executável por PUBLIC', async () => {
  const c = await runtime.connect()
  try {
    await assert.rejects(() => c.query('SELECT vazamento()'), (e) => e.code === '42501',
...
test('CATÁLOGO · atributos das três roles', async () => {
  ...
  try {
    ...
    assert.equal(por.app_login.rolinherit, false, 'app_login é NOINHERIT — a peça que faz funcionar')
  } finally { c.release() }
})
...
test('CATÁLOGO · o conjunto de memberships é exatamente o esperado', async () => {
  ...
  try {
    ...
    assert.deepEqual(real, ['app_login->app'],
      `membership inesperada: ${JSON.stringify(real)}`)
  } finally { c.release() }
})
...
test('BARREIRA · onConnect que falha destrói a conexão e rejeita o acquire', async () => {
  const ruim = new pg.Pool({
    host: HOST, port: PORT, database: DB,
    user: 'app_login', password: 'app_dev_only',
    max: 2, connectionTimeoutMillis: 5_000,
    onConnect: async (client) => { await client.query('SET ROLE inexistente') },
  })
  try {
    await assert.rejects(() => ruim.connect(), /inexistente|does not exist|42704/i,
...
test('ACHADO · onConnect é barreira por conexão FÍSICA — RESET ROLE sobrevive ao release', async () => {
  ...
  await comPoolDescartavel(async (pool) => {
    ...
    try {
      ...
      assert.equal(rows[0].current_user, 'app_login',
...
test('CONSERTO · SET LOCAL ROLE no UnitOfWork cura a conexão envenenada e reverte sozinho', async () => {
  ...
  await comPoolDescartavel(async (pool) => {
    ...
    try {
      ...
      assert.equal((await c.query('SELECT current_user')).rows[0].current_user, 'app',

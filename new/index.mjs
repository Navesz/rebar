 ...
 * entra se este arquivo estiver rodando de um lugar onde o `tooling/` não
...
async function main(argv) {
  ...
  //   node new/index.mjs <nome>                (do checkout)
  ...
  //
  // O `new/site/aplicar.mjs` copia os blocos dele com `cpSync(..., { force:
  ...
  if (!existsSync(caminhoSite)) {
    ...
    try {
      ...
      if (typeof aplicarSite !== 'function') {
        throw new Error('new/site/aplicar.mjs não exporta aplicarSite')
      }
  ...
  eco('   4. conferir .rebar-coauthors: entrou a identidade do git desta máquina.')
  eco('')
